##----------------------------------
##  This real data code for logistic regression with Elastic net penalty embeded in the Ensemble-based framework.
##  Ensemble_EN
##  Code Version 1.0
...
## 2.1 Colon dataset
#----------------
ensemblePanels <- lapply(colon_train_data, function(i){
  cvfit_ensem <- cv.glmnet(i, colon_train_lable, alpha=0.9,family="binomial",type.measure = "class")
})

ensembleValiPredction <- mapply(function(cvfit,x){
  valprediction <- predict(cvfit,x,type="response",s="lambda.min")
}, cvfit = ensemblePanels, x = colon_train_data)

ensembleTestPredction <- mapply(function(cvfit,x){
  tsprediction <- predict(cvfit,x,type="response",s="lambda.min")
}, cvfit = ensemblePanels, x = colon_test_data)
...
results.colon <- list("valpredmatrix" = ensembleValiPredction, "evlpredmatrix" = ensembleTestPredction, 
                      "Coef" = ensembleCoef, "NumbernonzeroCoef" = ensembleCoefNum)

##----------------
## 2.2 gbm dataset
#----------------
ensemblePanels <- lapply(gbm_train_data, function(i){
  cvfit_ensem <- cv.glmnet(i, gbm_train_lable, alpha=0.9,family="binomial",type.measure = "class")
})

ensembleValiPredction <- mapply(function(cvfit,x){
  valprediction <- predict(cvfit,x,type="response",s="lambda.min")
}, cvfit = ensemblePanels, x = gbm_train_data)

ensembleTestPredction <- mapply(function(cvfit,x){
  tsprediction <- predict(cvfit,x,type="response",s="lambda.min")
}, cvfit = ensemblePanels, x = gbm_test_data)
...
results.gbm <- list("valpredmatrix" = ensembleValiPredction, "evlpredmatrix" = ensembleTestPredction, 
                    "Coef" = ensembleCoef, "NumbernonzeroCoef" = ensembleCoefNum)

##----------------
## 2.3 Kidney dataset
#----------------
ensemblePanels <- lapply(kidney_train_data, function(i){
  cvfit_ensem <- cv.glmnet(i, kidney_train_lable, alpha=0.9,family="binomial",type.measure = "class")
})

ensembleValiPredction <- mapply(function(cvfit,x){
  valprediction <- predict(cvfit,x,type="response",s="lambda.min")
}, cvfit = ensemblePanels, x = kidney_train_data)

ensembleTestPredction <- mapply(function(cvfit,x){
  tsprediction <- predict(cvfit,x,type="response",s="lambda.min")
}, cvfit = ensemblePanels, x = kidney_test_data)
...
results.kidney <- list("valpredmatrix" = ensembleValiPredction, "evlpredmatrix" = ensembleTestPredction, 
                       "Coef" = ensembleCoef, "NumbernonzeroCoef" = ensembleCoefNum)

##----------------
## 2.4 Lung dataset
#----------------
ensemblePanels <- lapply(lung_train_data, function(i){
  cvfit_ensem <- cv.glmnet(i, lung_train_lable, alpha=0.9,family="binomial",type.measure = "class")
})

ensembleValiPredction <- mapply(function(cvfit,x){
  valprediction <- predict(cvfit,x,type="response",s="lambda.min")
}, cvfit = ensemblePanels, x = lung_train_data)

ensembleTestPredction <- mapply(function(cvfit,x){
  tsprediction <- predict(cvfit,x,type="response",s="lambda.min")
}, cvfit = ensemblePanels, x = lung_test_data)
...
results.lung <- list("valpredmatrix" = ensembleValiPredction, "evlpredmatrix" = ensembleTestPredction, 
                     "Coef" = ensembleCoef, "NumbernonzeroCoef" = ensembleCoefNum)
...
para.runs.colon = evaluate.ensemble.performance(results.colon, as.numeric(as.character(colon_train_lable)), 
                                                as.numeric(as.character(colon_test_lable)))

para.runs.gbm = list()
para.runs.gbm = evaluate.ensemble.performance(results.gbm, as.numeric(as.character(gbm_train_lable)), 
                                              as.numeric(as.character(gbm_test_lable)))

para.runs.kidney = list()
para.runs.kidney = evaluate.ensemble.performance(results.kidney, as.numeric(as.character(kidney_train_lable)), 
                                                 as.numeric(as.character(kidney_test_lable)))

para.runs.lung = list()
para.runs.lung = evaluate.ensemble.performance(results.lung, as.numeric(as.character(lung_train_lable)), 
