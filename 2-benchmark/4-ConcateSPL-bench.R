##----------------------------------
##  This real data code for self-paced learning with L1 penalty embeded in the concatenation-based framework.
##  Concate_SPL
##  Code Version 1.0
...
results <- selfpaced.single(X_train = combined_train_colon, 
                            Y_train = as.numeric(colon_train_lable),
                            X_test = combined_test_colon, 
                            Y_test = as.numeric(colon_test_lable),
...
## Record best result
results.colon <- list("best.valperf" = best_valperf, 
...
results <- selfpaced.single(X_train = combined_train_gbm, 
                            Y_train = as.numeric(gbm_train_lable),
                            X_test = combined_test_gbm, 
                            Y_test = as.numeric(gbm_test_lable),
                            lambda = sample.select,
                            uplambda = sample.add,
                            Iter_num = Iter_num)
...
## Record best result
results.gbm <- list("best.valperf" = best_valperf, 
                    "best.evlperf" = best_evlperf,
                    "best.coef" = best_coef, 
                    "best.numcoef" = best_numcoef)
...
results <- selfpaced.single(X_train = combined_train_kidney, 
                            Y_train = as.numeric(kidney_train_lable),
                            X_test = combined_test_kidney, 
                            Y_test = as.numeric(kidney_test_lable),
                            lambda = sample.select,
                            uplambda = sample.add,
                            Iter_num = Iter_num)
...
## Record best result
results.kidney <- list("best.valperf" = best_valperf, 
                       "best.evlperf" = best_evlperf,
                       "best.coef" = best_coef, 
                       "best.numcoef" = best_numcoef)
...
results <- selfpaced.single(X_train = combined_train_lung, 
                            Y_train = as.numeric(lung_train_lable),
                            X_test = combined_test_lung, 
                            Y_test = as.numeric(lung_test_lable),
                            lambda = sample.select,
                            uplambda = sample.add,
                            Iter_num = Iter_num)
...
## Record best result
results.lung <- list("best.valperf" = best_valperf, 
                     "best.evlperf" = best_evlperf,
                     "best.coef" = best_coef, 
                     "best.numcoef" = best_numcoef)
...
para.runs.colon = evaluate.SPL.performance(results = results.colon,
                                           y_train = as.numeric(colon_train_lable),
                                           y_test = as.numeric(colon_test_lable))

para.runs.gbm = list()
para.runs.gbm = evaluate.SPL.performance(results = results.gbm, 
                                         y_train = as.numeric(as.character(gbm_train_lable)), 
                                         y_test = as.numeric(as.character(gbm_test_lable)))

para.runs.kidney = list()
para.runs.kidney = evaluate.SPL.performance(results.kidney, 
                                            y_train = as.numeric(as.character(kidney_train_lable)), 
                                            y_test = as.numeric(as.character(kidney_test_lable)))

para.runs.lung = list()
para.runs.lung = evaluate.SPL.performance(results.lung,
                                          y_train = as.numeric(as.character(lung_train_lable)), 
                                          y_test = as.numeric(as.character(lung_test_lable)))
